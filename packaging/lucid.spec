# PyInstaller spec for lucid sidecar (Phase 1.7).
#
# Build:
#     pip install pyinstaller
#     pyinstaller --noconfirm packaging/lucid.spec
#
# Output:  dist/lucid/lucid.exe  (folder mode, lighter on AV scans than --onefile)
# 这个目录会被 Tauri 通过 tauri.conf.json `bundle.resources` 整体打进 .msi，
# 启动时 Rust 侧 (sidecar.rs::build_command) 会优先找
# `<resource_dir>/lucid/lucid.exe` 起子进程。
#
# Notes:
...
# Output to repo root dist/ so tauri.conf.json resource path ../../dist/lucid.exe works
import sys
...
hidden += collect_submodules("psutil")  # used by lucid.launchers for process detection
# Per-app registry: every `lucid/apps/<slug>.py` is a hot-plug module.
# We need PyInstaller to (a) bundle each module AND (b) emit them as .pyc files
# under MEIPASS/lucid/apps/ so `pkgutil.iter_modules` works at runtime.
...
_APPS_DIR = os.path.join(ROOT, "lucid", "apps")
hidden += collect_submodules("lucid.apps")
hidden += [
    f"lucid.apps.{os.path.splitext(fn)[0]}"
...
a = Analysis(
    [os.path.join(ROOT, "packaging", "lucid_entry.py")],
    pathex=[ROOT],
    binaries=[],
    datas=datas,
...
# NOTE: onefile mode — one self-extracting `dist/lucid.exe`. Tauri's
