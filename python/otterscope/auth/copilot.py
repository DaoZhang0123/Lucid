...
1. **GitHub OAuth token** (long-lived, stored)
   ...
   - Saved to `state_file` (default `%LOCALAPPDATA%\\dev.otterscope\\copilot.json`).
...
def _default_state_file() -> Path:
    """`%LOCALAPPDATA%\\dev.otterscope\\copilot.json` on Windows，其他平台用 ~/.otterscope/copilot.json"""
    if os.name == "nt":
        base = os.environ.get("LOCALAPPDATA") or str(Path.home() / "AppData" / "Local")
        return Path(base) / "dev.otterscope" / "copilot.json"
    return Path.home() / ".otterscope" / "copilot.json"
