...
> **第二个演示** —— 语音输入 + 精确点击。长按空格说话，Lucid 用本地 Whisper 转写后，全程靠视觉驱动微信（不调微信 API）。
...
语音（长按空格）：  “打电话给爸爸”
          ↓
Lucid：  转写 → launch_app("微信")
          → find("爸爸") → click(命中的联系人)
          → click(语音通话按钮) → 等待呼叫接通
          → “完成。已在微信向爸爸发起语音通话。”
```

> **第三个演示** —— 办公场景：Lucid 听到指令后读桌面文档、写出 PPT 并回传。

https://github.com/user-attachments/assets/7841cec9-58bc-49fd-b87a-5a0a27529fd6

```
语音 / 聊天：  “我桌面有个文档，你根据那个文档写一个 PPT，然后写完了发给我”
          ↓
Lucid：  扫描桌面 → 定位源文档 → 读取内容
          → launch_app("PowerPoint") → 根据文档提纲逐页填充
          → 保存为 .pptx → 回到对话 → 附件发送给用户
          → “完成。PPT 已生成并发送。”
```
