...
### 对自己诚实
- 每次运行落盘 `~/.lucid/logs/threads/<thread>/` —— `events.jsonl`、`messages.json`、所有截图、完整的 LLM context dump。
...
> *"读 `~/.lucid/config.toml`，告诉我现在用的是哪个 LLM provider。"*（走 `read_file` meta tool，不动 GUI。）
...
如果出现 `missing api_key (config .api_key or LITELLM_MASTER_KEY environment variable)`，请在 `~/.lucid/config.toml` 里设置 `[llm.proxy].api_key`，或导出 `LITELLM_MASTER_KEY` 环境变量。
