...
<p align="center">
  <em><strong>名字从哪来？</strong> <em>明眸</em>——出自“明眸善睞”，明亮、洞察、看得清。吉祥物是一只小螃蟹：横着走路，也始终睁大眼睛盯着屏幕——这正是 Lucid 干的事。</em>
</p>
...
完整的 auto-reply 过程：任务栏 UIA 监听捕获新消息 → `launch_app` 拉起聊天客户端 → 视觉驱动点击定位到对话 → agent 输入回复并回车发送。全程不走 MCP、不调 API，全部走你真实的客户端。
...
![Lucid 架构](image/architecture_Chinese.png)
...
| **LLM 后端** | GitHub Copilot · Anthropic · OpenAI（及任何 OpenAI 兼容的 base URL）· Google Gemini |
| **模型（实测效果排序）** | **Claude Opus 4.7** ≈ **GPT-5.5** &nbsp;&gt;&nbsp; Claude Opus 4.6 &nbsp;&gt;&nbsp; Gemini 3.5 Flash |
