...
- **AndroidWorld**：移动端，仅作参考。

> 本项目 MVP 出炉后应跑这两个基准至少一遍，证明"通用性 + 准确率"在可接受区间。
...
- **可复用的开源积木**：
  - Grounding：UI-TARS-7B / OS-Atlas / ShowUI（提升点击精度，可选本地部署）。
  - 输入：pyautogui / nut.js / mediar-ai/terminator（fallback）。
  - 评测：OSWorld、WindowsAgentArena 直接接入。
  - 截图 / 录制：OpenAdapt 录制思路、Cradle 长时序记忆。
- **差异化护城河**：
