...
| 5 | **[simular-ai/Agent-S](https://github.com/simular-ai/Agent-S)** | ~10.9k | Python 框架（S1/S2/S3，pip `gui-agents`） | 纯视觉 + 独立 Grounding 模型 | **OSWorld 首个超人类（72.6%，2025-12）**。S3 反而**抛弃了 S2 的 Manager/Worker 分层**，回到"单 Worker + reflection"更简单更快；Grounding 完全外置（推荐 `UI-TARS-1.5-7B` 跑 HF Inference Endpoint，1920×1080 出坐标）；可选 `CodeAgent` 直跑 Py/Bash（明确 ⚠️ arbitrary code）；**bBoN**（Behavior Best-of-N，跑 N 次 rollout 选最好）把单跑 66% → 72.6%；procedural memory 按 OS 切换可用动作。可借鉴：①推理 / grounding 双模型分工 ②bBoN 用在 OtterScope 的 `confirm_critical` 关键步骤上 ③扁平 ReAct 优于多层（与本项目路线吻合）。差距：单显示器、纯 CLI、无 Tauri / 托盘 / IME / 中文场景 / 任务栏自动回复。 |
...
### 1.b 反向赛道：Agent-Native CLI（非视觉路线，可作 OtterScope 的工具库）

| # | 项目 | Stars 量级 | 形态 / 技术栈 | 路线 | 与本项目关系 |
| --- | --- | --- | --- | --- | --- |
| C1 | **[HKUDS/CLI-Anything](https://github.com/HKUDS/CLI-Anything)** | 较新但快速增长 | Click 生成器 + 7-Phase Harness + 插件市场 | **不点击 GUI，而是给每个 App 用 codegen 生成专用 CLI** | **与本项目互补，不竞争**。它的方法论：Analyze → Design → Implement → Plan Tests → Write Tests → Document → Publish；以插件形式分发到 Claude Code / Pi / OpenClaw / OpenCode / Codex / Qodercli / Goose / Copilot CLI；**[CLI-Hub](https://hkuds.github.io/CLI-Anything/)** 已收录 50+ 现成 CLI（Blender、GIMP、Krita、OBS、Audacity、Kdenlive、Shotcut、MuseScore、LibreOffice、FreeCAD、QGIS、Godot、Inkscape、ComfyUI、Ollama、Zotero、Obsidian、NotebookLM、Zoom、Safari…），2269 测试用例。每个 CLI 自带 `SKILL.md`（agent 可发现）+ 统一 REPL + JSON / human 双输出。**对 OtterScope 的价值**：① `apps/<slug>.py` 演化为 `apps/<slug>/SKILL.md` + helper，agent 可发现 ② OtterScope 启动时探测 `cli-anything-*` 包并自动暴露为 tool，**有现成 CLI 的 App 永远走 CLI**（更快/更稳/更便宜，把视觉路线留给真正非结构化的场景） ③ 7-phase 思想反向用于 doze：发现重复点击模式 → 自动落地为 `apps/<slug>.py` helper ④ tool 输出统一为 `text` + `json` 双格式。 |

## 2. 移动端 GUI Agent（横向参考）

虽然 OtterScope 聚焦 Windows 桌面，移动端 GUI Agent 在过去两年的进展（多 Agent 协作、
...
| # | 项目 | 平台 | 技术亮点 | OtterScope 可借鉴 |
| --- | --- | --- | --- | --- |
| M1 | [X-PLUG/MobileAgent](https://github.com/X-PLUG/MobileAgent)（含 v1–v3.5 / PC-Agent / Mobile-Agent-E） | Android / Win / Mac | 多 Agent 协作（planner+decision+reflection+memory）；GUI-Owl 1.5 自研基模；PC-Agent 主动感知 | **多 Agent 拆分** 和 **PC-Agent 的主动感知模块**最值得移植到 OtterScope 的 ReAct 循环 |
| M2 | [IPADS-SAI/MobiAgent](https://github.com/IPADS-SAI/MobiAgent) | Android（含 on-device） | Decider/Grounder 双模型；AgentRR 录-放加速；MobiFlow milestone DAG 评测；user profile (Mem0+GraphRAG) | **AgentRR 录放思想** 与 **user profile memory** 直接对应 OtterScope 的 templates / memory 路线 |
| M3 | [TencentQQGYLab/AppAgent](https://github.com/TencentQQGYLab/AppAgent) (CHI 2025) | Android | 两阶段：Exploration 期生成"元素文档"，Deployment 期查文档；网格覆盖兜底 grounding | **两阶段：先探索写文档、再上岗"** 等价于 OtterScope 未来的 "App 区域库 + 学习反馈" 闭环 |
| M4 | [minitap-ai/mobile-use](https://github.com/minitap-ai/mobile-use) | Android / iOS | LangGraph 多节点（cortex / executor / planner / hopper / contextor / outputter / summarizer）；100% AndroidWorld | **LangGraph 节点化** 对应 OtterScope 未来的 planner/executor/checker 三角 |
| M5 | [droidrun/mobilerun](https://github.com/droidrun/mobilerun) | Android / iOS | 多 LLM provider；CLI 优先；Arize Phoenix tracing；macro / app_cards 模块 | **app_cards** 与 OtterScope 的 "App 区域库 + launchers.json" 思路高度一致；**Phoenix tracing** 可作为 events.jsonl 的可视化升级 |
...
- **移动端经验沉淀** 见 [mobile-agents-analysis.md](mobile-agents-analysis.md)，可平移的关键模式：
  ...
  - app_cards / launchers.json（mobilerun、OtterScope planned）
  - **Behavior Best-of-N**（Agent-S3 把 OSWorld 66% → 72.6%）：在 `confirm_critical` 关键步骤上跑 N 次投票
  - **Agent-Native CLI 复用**（CLI-Anything Hub）：开机探测本机 `cli-anything-*` 包自动注册为 tool，凡是有现成 CLI 的 App 永不走视觉路线
- **可复用的开源积木**：
