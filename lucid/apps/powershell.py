...
- [seed · script-fallback] If a long inline command becomes fragile (especially with `$` variables, nested quotes, or backticks), write a temporary `.ps1` file and run it with `powershell -NoProfile -ExecutionPolicy Bypass -File "<path>.ps1"`. This is usually more reliable than fighting escaping in a single line.
