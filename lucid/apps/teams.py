...
- [seed · unread-badge-scan] **"Count unread" / "report unread badges" tasks must visually scan EVERY tab in the left rail (Activity / Chat / Teams / Calendar / Calls / Files), NOT just the currently-active tab.** Teams renders a tiny red numeric badge in the top-right corner of each tab's icon (e.g. "3", "99", "99+"). The currently-selected tab can show `unread=0` while another tab carries `99` — answering `unread=none` because the active tab has no badge is a partial-fail (E2E 20260520-003613 M3). Workflow: take ONE L2 of the active window → in your assistant_text transcribe each rail tab's icon + any badge digit you can see (e.g. "Activity: no badge · Chat: 3 · Teams: 99 · Calendar: no badge") → sum / report.
