...
- [seed · long-install] **Package installs (`npm install`, `pip install`, `winget install`, `cargo install`, `go install`, `dotnet add package`, `choco install`, `scoop install`, `gem install`) routinely take 30–300s and WILL time out under `run_shell`'s default 20s.** Two acceptable paths: (a) bump `timeout_s` to the hard cap of 300 if you believe the install fits (`run_shell` is capped at 300s no matter what you pass — there is no point asking for more); (b) for anything you expect to exceed 300s (global npm packages with native deps, conda envs, large pip wheels, winget downloads on slow networks), **do NOT use `run_shell` at all** — `write_file` a `.bat` driver that does the work and writes a sentinel file when finished (e.g. `install.ok`), `launch_app(name='cmd')`, `type` the bat's absolute path + Return, then poll the sentinel with short `run_shell` checks. **Bat-driver rules (every line matters — E2E 20260518-235713 lost ~5 min violating these):** (i) `write_file` the bat with mode='overwrite' (Lucid auto-converts LF→CRLF for `.bat`/`.cmd` extensions; older builds did not — verify your bat with `Get-Content -Encoding Byte | Format-Hex` if anything looks off). (ii) First line: `@echo off`. (iii) Second line: `if not exist "<workdir>" mkdir "<workdir>"` (always create before cd; absent dir + LF → silent cd fail → npm runs in `%USERPROFILE%` and pollutes home `node_modules`). (iv) Third line: `cd /d "<workdir>" || exit /b 1` (the `|| exit /b 1` prevents subsequent lines from running in the wrong cwd). (v) For npm: `call npm init -y >nul 2>nul` BEFORE `call npm install <pkg>` — without `init` npm walks parents looking for `package.json`, may install into a parent dir, and `node script.js` in `<workdir>` then resolves modules from an unexpected location. (vi) Final line: `echo DONE > "<workdir>\\install.ok"`. (vii) Sentinel verification must check **both** the `.ok` file AND `Test-Path "<workdir>\\node_modules\\<pkg>\\package.json"` — the `.ok` file alone is forgeable by partial runs. The pattern `npm list -g <pkg> 2>nul || npm install -g <pkg>` inside `run_shell(timeout_s=60)` is the canonical failure mode (E2E 20260518-191942: 24min cancelled at step 6). **Do NOT pre-check with `npm list` / `pip show` / `npm ls` / `npm root -g`** — those query commands are themselves slow on Windows (`npm list -g` and `npm root -g` walk the global prefix and routinely take 30-90s, blowing the default 20s timeout even when nothing is installed; E2E 20260518-201950 and 20260518-212634).
- [seed · npm-prefer-local] **For npm packages, STRONGLY prefer a local install over `-g` global.** Reason: global npm prefix varies by Node installer (WinGet → `...\\WinGet\\Packages\\OpenJS.NodeJS_*\\node-v*-win-x64\\node_modules`; official MSI → `%APPDATA%\\npm\\node_modules`; Scoop/Chocolatey → yet other paths). There is NO single `Test-Path` you can rely on to verify a global install, and `node script.js` won't find globally-installed modules without `NODE_PATH` set to the right prefix (which you don't know without calling slow `npm root -g`). Local install solves all of this: pick a known working dir (e.g. `cd %TEMP%\\lucid-pptx && npm install pptxgenjs`), then `node script.js` from that same dir resolves `require("pptxgenjs")` automatically. To verify a local install: `Test-Path "<workdir>\\node_modules\\<pkg>\\package.json"` — ms-fast, deterministic. Only fall back to `-g` if the user explicitly asks for a system-wide install.
...
- [seed · run-shell-cmd-quote] **`run_shell(shell="cmd", command='...')` mangles paths that contain embedded `"` quotes.** Python's `subprocess` wraps the command via `list2cmdline` which escapes inner `"` as `\"` — but cmd.exe does **not** understand `\"` as a quote escape (it only does `""`). So `start <exe> /flag "C:\path with spaces\file.ext"` reaches the launched program as `\"C:\path...\file.ext\"` with the literal backslash-quote characters in argv, and the program fails with errors like `无法读取 "C:\…\file.ext"` / `Cannot open "C:\…"` (E2E 20260519-164242: PowerPoint refused to open a freshly-written .pptx for 3+ retries). **Fix — use PowerShell's `Start-Process -ArgumentList` for any "launch <exe> with a quoted path" task:** `run_shell(shell="powershell", command="Start-Process <exe> -ArgumentList '/flag','<absolute_path>'")`. PowerShell builds the child process's argv directly with .NET, no extra cmd parse, no quote escapes — each `-ArgumentList` element becomes one argv entry. Use single quotes inside the PS string so embedded backslashes stay literal. This rule applies to **every** GUI launcher you'd reach for via `run_shell`: `powerpnt`, `winword`, `excel`, `notepad`, `mspaint`, `explorer`, custom .exes, etc.
...
- [seed · skill-reference-files] **Anthropic-style skills are a folder, not a single file.** `SKILL.md` is just the index — the real API examples / templates / helper scripts live in sibling files inside the same folder (e.g. a `pptx` skill ships `pptxgenjs.md`, `editing.md`, `scripts/*`; a `docx` skill ships its own references; many community skills do the same). The `read_skill` response lists what's available under "Reference files in this skill". To load one, pass `file=<relative path>`: `read_skill(name='<skill>', file='<reference>.md')` (or the shorthand `read_skill(name='<skill>/<reference>.md')`). **Never call `read_skill(name='<reference>.md')`** — a reference file is not a skill on its own and the call will fail with "skill not found". This rule is universal: whenever a SKILL.md body tells you to "see X.md" / "run scripts/Y", that's a sibling — fetch it with the `file=` form, don't go searching the web or guessing the API.
- [seed · office-modal] **When `focus_window(title_substring=<office-app>)` returns a tiny window (active_window screenshot is smaller than ~600×400 px), you are looking at a modal error / warning dialog, NOT the main app.** PowerPoint, Word, and Excel all reuse the parent title ("Microsoft PowerPoint" / "...Word" / "...Excel") on their child error dialogs ("无法打开此文件" / "Cannot open" / "Protected View" / "File association"), so `focus_window` cheerfully matches them. Symptom: every screenshot looks identical, your clicks at the "same" coordinates achieve nothing, and the agent enters a rewrite-and-retry loop (E2E 20260518-235713 — ~22 min lost reopening a stale pptx because the agent kept dismissing the same error modal). **Required workflow when active_window comes back ≲ 600×400 and the title is an Office app:** (1) **read the dialog text from the L2 screenshot** before clicking anywhere — "找不到文件" / "Cannot find" means a stale path, "受保护的视图" / "Protected View" needs an "启用编辑" / "Enable Editing" click, "格式不正确" / "incorrect format" means the file is actually corrupt; (2) if it's a protected-view or trust prompt, press `key text='alt+e'` (启用编辑 / Enable Editing accelerator) rather than pixel-clicking; (3) if it's a "cannot open / corrupt" dialog, `key text='Escape'` to dismiss and then **verify the file path you're trying to open actually changed since the last regeneration** (`Get-Item <path> | Select Length, LastWriteTime`) — opening a stale file from a previous session is the #1 cause of "PowerPoint keeps rejecting my pptx" loops; (4) only after the modal is closed and the main app window (full-size, title ends in ` - PowerPoint` / similar) is focused should you start interacting with content.
"""
